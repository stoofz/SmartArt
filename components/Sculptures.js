...
const Sculptures = () => {
  ...
  useEffect(() => {
    const getSculptures = async () => {
      try {
        const response = await fetch('/api/listSculptures');
        if (response.ok) {
          const sculptures = await response.json();
          const sculptureData = () => sculptures.map((sculpture) => {
            setSculptures(sculpture.products);
            setTotalProducts(sculpture.products.length);
          })
          sculptureData();
        }
      } catch (error) {
  ...
  const pageSculptures = sculptures.slice(firstProductOfPage, lastProductOfPage);
  ...
  const sculptureList = () => (pageSculptures.map((product) =>
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      >
        ...
        {totalProducts === 0 ? (<div>No results found </div>) : (sculptureList())}
