...
const Photography = () => {
  ...
  useEffect(() => {
    const getPhotography = async () => {
      try {
        const response = await fetch('/api/listPhotography');
        if (response.ok) {
          const photography = await response.json();
          const photographyData = () => photography.map((photography) => {
            setPhotography(photography.products);
            setTotalProducts(photography.products.length);
          })
          photographyData();
        }
      } catch (error) {
  ...
  const pagePhotography = photography.slice(firstProductOfPage, lastProductOfPage);
